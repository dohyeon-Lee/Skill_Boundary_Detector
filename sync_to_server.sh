#!/usr/bin/env bash
# Send ignored dataset/model/output folders from this SBD workspace to another server.
#
# Usage:
#   ./sync_to_server.sh
#   REMOTE=user@host REMOTE_BASE=/path/to/SBD ./sync_to_server.sh
#
# Flow:
#   1. Pick one top-level SBD folder from the large-data/output .gitignore set.
#   2. Send that folder as-is, or keep drilling down into subfolders.
#   3. The chosen folder is synced to the same relative path under REMOTE_BASE.
...
list_top_dirs() {
    local dirs=()
    local pattern path name existing
    ...
    for pattern in "${SYNC_ROOT_PATTERNS[@]}"; do
        if [[ "${pattern}" == *'*'* ]]; then
            ...
            done < <(compgen -G "${LOCAL_BASE}/${pattern}" || true)
        else
            path="${LOCAL_BASE}/${pattern}"
            [ -d "${path}" ] || continue
            name="${pattern}"
            existing=0
            for existing_name in "${dirs[@]}"; do
...
choose_folder() {
    ...
    [ "${#top_dirs[@]}" -gt 0 ] || die "No output/dataset folders found under ${LOCAL_BASE}"

    print_menu "보낼 최상위 폴더 선택 (${LOCAL_BASE})" "${top_dirs[@]}"
    local idx
    ...
    while true; do
        local abs="${LOCAL_BASE}/${rel}"
        [ -d "${abs}" ] || die "Local folder not found: ${abs}"
        ...
        case "${action}" in
            ...
            2)
                local children=()
                mapfile -t children < <(list_child_dirs "${rel}")
...
sync_folder() {
    ...
    local dst="${REMOTE}:${REMOTE_BASE}/${rel}"

#!/usr/bin/env bash
# /data2/dohyeon 바로 아래 상위 폴더별 디스크 사용량 요약
#
# 사용법:
#   ./check_dohyeon_disk_usage.sh
#   MIN_SIZE=1G ./check_dohyeon_disk_usage.sh      # 1G 미만 숨기기
#   INCLUDE_HIDDEN=0 ./check_dohyeon_disk_usage.sh # 숨김 폴더 제외
#
# 환경변수:
#   ROOT            기준 경로 (기본: /data2/dohyeon)
#   MIN_SIZE        이 값보다 작은 폴더 숨기기, e.g. 1G (기본: 0, 전체 표시)
#   INCLUDE_HIDDEN  숨김 폴더 포함 여부: 1 포함, 0 제외 (기본: 1)

set -euo pipefail

ROOT="${ROOT:-/data2/dohyeon}"
MIN_SIZE="${MIN_SIZE:-0}"
INCLUDE_HIDDEN="${INCLUDE_HIDDEN:-1}"

if [[ ! -d "$ROOT" ]]; then
    echo "ROOT 경로가 없습니다: ${ROOT}" >&2
    exit 1
fi

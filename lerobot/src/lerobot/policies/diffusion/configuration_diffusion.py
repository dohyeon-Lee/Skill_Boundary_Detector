...
class DiffusionConfig(PreTrainedConfig):
    ...
    def _migrate_legacy_config(cls, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if legacy_eef_enabled:
            raise ValueError(
                "This Diffusion checkpoint enables the removed EEF-relative action mode and cannot be loaded "
                "by the current policy implementation."
            )

        for key in (
            "use_eef_relative_actions",
            "eef_relative_stats_path",
            "eef_position_scale",
            "eef_rotation_scale",
        ):

...
class SkillPredictor(nn.Module):
    """Transformer decoder that predicts the next skill latent z_k.
    ...
    tokens and contextual Paligemma prefix hidden states, preserving token-level
    ...
    def forward(
        ...
        query_output = decoded_queries[:, -self.num_query_tokens :]
        attended_prefix = self.output_norm(query_output).mean(dim=1)
        return self.out_mlp(attended_prefix)

...
def aggregate_datasets(
    ...
    Args:
        ...
        concatenate_videos: Whether to remux input videos into size-bounded output files. If False,
            every input MP4 is copied to a separate output file. This avoids timestamp incompatibilities
            when source datasets were encoded with different H.264 encoders or B-frame settings.
    """
...
def aggregate_videos(
    ...
    Args:
        ...
        video_files_size_in_mb: Maximum size for video files in MB (defaults to DEFAULT_VIDEO_FILE_SIZE_IN_MB)
        chunk_size: Maximum number of files per chunk (defaults to DEFAULT_CHUNK_SIZE)
        concatenate_videos: Whether compatible source MP4s may be remuxed into the current output file.
            If False, each source MP4 is copied to a new destination file after the first one.

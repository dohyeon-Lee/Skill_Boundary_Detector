...
"""Relative/absolute action conversion steps, ported from upstream lerobot.

Newer hub checkpoints (e.g. ``lerobot/pi05_base`` since 2026-06-03) save these steps in
their processor configs (typically with ``enabled: false``), so they must exist in the
registry for those configs to deserialize. Mirrors OpenPI's DeltaActions/AbsoluteActions
transforms: relative = action - state during preprocessing, reversed in postprocessing.
"""
...
class RelativeActionsProcessorStep(ProcessorStep):
    ...
    def get_cached_state(self) -> torch.Tensor | None:
        """Return the cached ``observation.state`` used as the reference point for the conversion."""
...
class AbsoluteActionsProcessorStep(ProcessorStep):
    """Converts relative actions back to absolute actions (action += state) for masked dimensions.
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        mask = self.relative_step._build_mask(action.shape[-1])  # noqa: SLF001

...
* ``prune_local`` (``--prune-local``; on by default on RunPod via ``hf_watch_prune_local``) applies the
  ...
  newest step (resume), steps still being written, protected steps and linked start checkpoints are
  never removed. Without ``--keep`` nothing is pruned.
"""
...
def local_steps(project_root: Path, roots: list[str]) -> dict[str, Path]:
    """{repo path: step dir} for every complete local step."""
    found = {}
    for root, run_dir in find_runs(project_root, roots):
        for step in complete_steps(run_dir):
            found[f"{root}/{run_dir.relative_to(project_root / root).as_posix()}/checkpoints/{step.name}"] = step
    return found
...
def plan_sync(local: set[str], remote: set[str], keep: int, protect: set[int]) -> tuple[list[str], list[str], list[str]]:
    """(upload, delete on the Hub, prune locally): the per-run window spans local and Hub steps, so a
    step dropped for age is not re-uploaded on the next scan or after a restart, and both sides keep
    the same steps."""
    outside = set(plan_cleanup(local | remote, keep, protect))
    return sorted(local - remote - outside), sorted(remote & outside), sorted(local & outside)
...
def watch(project_root: Path, repo: str, *, root: str | None = None, interval: int = 300, once: bool = False,
    ...
    first = True
    while True:
        found = local_steps(project_root, roots)
        to_upload, to_delete, to_prune = plan_sync(set(found), uploaded, keep, protect)
        if not prune_local:
            to_prune = []
        ...
        if to_prune:
            ...
            for path in to_prune:
                print(f"    {found[path]}")
        if first and (to_upload or to_prune) and not assume_yes and not dry_run:

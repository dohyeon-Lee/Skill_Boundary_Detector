...
<section class="section"><h2>Checkpoint trend</h2><div class="toolbar"><label class="control">Metric <select id="trendMetric"></select></label></div><div class="scroll" id="trend"></div></section>
...
const colors=['#2878b5','#d95f59','#2a9d8f','#9b5de5','#e09f3e','#577590','#43aa8b','#f3722c'];let modelIndex=0,checkpointTag=null;
...
function renderTrend(){const key=trendSelect.value||Object.keys(metricDefs)[0],definition=metricDefs[key],series=DATA.models.map((item,index)=>({item,index,points:item.checkpoints.map(cp=>[epoch(cp.epoch_tag),value(cp,key)]).filter(point=>Number.isFinite(point[0])&&point[1]!=null)})).filter(item=>item.points.length);if(!series.length){document.getElementById('trend').innerHTML='<div class="empty">No values.</div>';return}const all=series.flatMap(item=>item.points),xmin=Math.min(...all.map(p=>p[0])),xmax=Math.max(...all.map(p=>p[0])),rawMin=Math.min(...all.map(p=>p[1])),rawMax=Math.max(...all.map(p=>p[1])),padding=(rawMax-rawMin||1)*.08,ymin=Math.min(0,rawMin-padding),ymax=rawMax+padding,width=1050,height=300,left=55,right=20,top=20,bottom=48,pw=width-left-right,ph=height-top-bottom,x=x=>left+(x-xmin)/Math.max(xmax-xmin,1)*pw,y=y=>top+(ymax-y)/Math.max(ymax-ymin,1e-9)*ph;let svg=`<svg class="chart" viewBox="0 0 ${width} ${height}">`;for(let i=0;i<=4;i++){const yy=top+i*ph/4,val=ymax-i*(ymax-ymin)/4;svg+=`<line x1="${left}" y1="${yy}" x2="${width-right}" y2="${yy}" stroke="#dde3eb"/><text x="${left-7}" y="${yy+4}" text-anchor="end" font-size="10" fill="#667085">${definition[2]==='pct'?(val*100).toFixed(1)+'%':val.toFixed(3)}</text>`}series.forEach(({item,index,points})=>{const path=points.map((point,i)=>`${i?'L':'M'}${x(point[0]).toFixed(1)},${y(point[1]).toFixed(1)}`).join(' ');svg+=`<path d="${path}" fill="none" stroke="${colors[index%colors.length]}" stroke-width="2.2"/>`;points.forEach(point=>svg+=`<circle cx="${x(point[0])}" cy="${y(point[1])}" r="3" fill="${colors[index%colors.length]}"><title>${esc(item.name)} · epoch ${point[0]} · ${format(point[1],definition[2])}</title></circle>`)});svg+=`<line x1="${left}" y1="${top+ph}" x2="${width-right}" y2="${top+ph}" stroke="#667085"/>`;[xmin,Math.round((xmin+xmax)/2),xmax].forEach(t=>svg+=`<text x="${x(t)}" y="${height-25}" text-anchor="middle" font-size="10" fill="#667085">${t}</text>`);series.forEach(({item,index},rank)=>svg+=`<rect x="${left+rank*220}" y="${height-13}" width="10" height="10" fill="${colors[index%colors.length]}"/><text x="${left+14+rank*220}" y="${height-4}" font-size="10" fill="#344054">${esc(item.name)}</text>`);document.getElementById('trend').innerHTML=svg+'</svg>'}

"""Build episode-exact GT skill records directly from FSQ encoding artifacts."""
...
def load_fsq_episode_data(
    ...
    episode_rows = load_episodes(Path(raw_dataset_dir))
    episode_meta = {
    ...
    for row in rows:
        ...
        dataset_start = int(episode_meta[episode]["dataset_from_index"])
        goal_index = dataset_start + row["frame_end"] - 1
...
class GoalImageStore:
    """Lazy worker-local cache of demonstration endpoint images for oracle-C."""

    def __init__(self, raw_dataset_dir: str | Path):
    ...
    def get(self, indices: torch.Tensor | list[int]) -> tuple[torch.Tensor, torch.Tensor]:
        values = [int(value) for value in (indices.tolist() if isinstance(indices, torch.Tensor) else indices)]
        for index in values:
            if index not in self._cache:
                item = self.dataset[index]
                self._cache[index] = (
                    item["observation.images.image"].detach().cpu(),
                    item["observation.images.wrist_image"].detach().cpu(),
                )
        third = torch.stack([self._cache[index][0] for index in values])
        wrist = torch.stack([self._cache[index][1] for index in values])
        return third, wrist

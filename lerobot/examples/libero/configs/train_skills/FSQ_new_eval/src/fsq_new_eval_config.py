#!/usr/bin/env python3
"""Resolve standalone FSQ_new closed-loop eval settings."""
...
DEFAULT_CONFIG_PATH = _HERE.parent.parent / "fsq_new_eval_config.yaml"
...
def _modes(value: object) -> tuple[str, ...]:
    aliases = {"a": "a", "b": "b", "c": "c", "image": "b", "goal": "c"}
    raw = [item.strip().lower() for item in str(value or "a,b,c").split(",") if item.strip()]
    modes = tuple(aliases.get(item, "") for item in raw)
    if not modes or any(not item for item in modes) or len(set(modes)) != len(modes):
        raise ValueError(f"modes must be a duplicate-free comma list from a,b,c; got {value!r}.")
    return modes
...
def _checkpoint(run_dir: Path, value: object) -> tuple[Path, str]:
    ...
    if raw == "last":
        periodic = sorted(run_dir.glob("FSQ_epoch*.pt"))
        return (periodic[-1], periodic[-1].stem.removeprefix("FSQ_epoch")) if periodic else (run_dir / "FSQ.pt", "best")
    if raw.isdigit():
        tag = f"{int(raw):04d}"
...
def _skillset_dir(meta: dict, fsq_dataset_root: Path, source: str) -> Path:
    ...
    min_skills = int(meta.get("skillset_min_skills", 2))
    minimum = "" if min_skills == 2 else f"_ms{min_skills}"
    custom = str(meta.get("skillset_output_suffix", "") or "").strip()
...
def build_settings(cfg: dict) -> dict:
    ...
    fsq_outputs = outputs_root / "FSQ_new"
    dino_model_path = resolve_path(project_root, get_value(cfg, "dino_model_path", "models/dinov3-vitl16"))

    models_yaml = get_value(cfg, "models", None)
    if not isinstance(models_yaml, list) or not models_yaml:
        raise ValueError("Set models to a non-empty list of {run, checkpoint, modes, label} entries.")
    ...
    for model_index, entry in enumerate(models_yaml):
        ...
        if not run:
            raise ValueError("Each models[] entry needs run: <FSQ_new output folder name>.")
        run_dir = Path(run).expanduser() if Path(run).expanduser().is_absolute() else fsq_outputs / run
        meta_path = run_dir / "fsq_meta.json"
        if not meta_path.is_file():
            raise FileNotFoundError(f"FSQ_new metadata not found: {meta_path}")
        meta = json.loads(meta_path.read_text())
        model_path, checkpoint_tag = _checkpoint(run_dir, get_value(entry, "checkpoint", "best"))
        if not model_path.is_file():
            raise FileNotFoundError(f"FSQ_new checkpoint not found: {model_path}")
        skills_dir = _skillset_dir(meta, fsq_dataset_root, source)
        if not skills_dir.is_dir():
            raise FileNotFoundError(f"FSQ skill NPZ directory not found: {skills_dir}")
        latents_path = run_dir / f"skill_latents_eval_{checkpoint_tag}.npz"
        advance = str(get_value(entry, "advance_mode", get_value(cfg, "advance_mode", "terminator"))).lower()
        ...
        for mode in _modes(get_value(entry, "modes", get_value(cfg, "modes", "a,b,c"))):
            display = f"{model_label} [{mode.upper()}]"
            key = _safe(display)
            if key in used_keys:
    ...
    settings = {
        ...
        "original_dataset_dir": project_root / "libero_original_dataset" / str(get_value(cfg, "target_task", "libero_90")),
        ...
        "skill_end_progress_threshold": float(get_value(cfg, "skill_end_progress_threshold", 0.95)),
        "inference_skill_max_length": int(get_value(cfg, "inference_skill_max_length", 150)),
        ...
        "wandb_project": str(get_value(cfg, "wandb_project", "FSQ_new_eval")),
        "wandb_run_name": f"FSQnewEval_{_safe(output_name)}",
        "eval_partition": part,

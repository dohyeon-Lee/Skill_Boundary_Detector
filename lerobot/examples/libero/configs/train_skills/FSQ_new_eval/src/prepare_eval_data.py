#!/usr/bin/env python3
"""Encode FSQ skill NPZs into an episode-addressable eval cache."""
...
from FSQ_new import load_fsq_encoder  # noqa: E402
from train_FSQ_new import load_skill_files  # noqa: E402
...
def _cache_matches(path: Path, fingerprint: dict[str, object]) -> bool:
    ...
    try:
        with np.load(path, allow_pickle=False) as data:
            ...
            required = {"tokens", "episode_id", "task_id", "skill_index", "frame_start", "frame_end", "length"}
...
def prepare(model_path: Path, skills_dir: Path, output_path: Path, device: str) -> Path:
    ...
    if _cache_matches(output_path, fingerprint):
        print(f"[FSQ-new eval] cache current: {output_path}")
        return output_path
    ...
    latents, tokens = [], []
    for segment in tqdm(segments, desc="Encoding FSQ-new eval skills"):
    ...
    print(f"[FSQ-new eval] wrote {len(tokens)} encoded skills -> {output_path}")
    return output_path

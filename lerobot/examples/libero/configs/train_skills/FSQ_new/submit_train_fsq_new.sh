...
CONFIG_PATH="${TRAIN_SKILLS_CONFIG:-${SCRIPT_DIR}/fsq_new_config.yaml}"
...
eval "$("${BOOTSTRAP_PYTHON}" "${FSQ_SRC_DIR}/resolve_fsq_new_config.py" --config "${CONFIG_PATH}")"
...
echo "Submit FSQ-new train (A/B/C regimes)"
...
TRAIN_SKILLS_CONFIG="${CONFIG_PATH}" TRAIN_DATA="${TARGET_DATASET}" \
  sbatch "${SBATCH_ARGS[@]}" "${FSQ_SRC_DIR}/train_fsq_new.sbatch"

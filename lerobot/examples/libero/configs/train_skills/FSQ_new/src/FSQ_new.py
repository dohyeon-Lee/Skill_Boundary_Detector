...
class ContextResamplerLayer(nn.Module):
    """Latent self-attention followed by cross-attention to fixed visual memory."""

    def __init__(self, width: int, n_heads: int, dropout: float):
        super().__init__()
        self.self_norm = nn.RMSNorm(width)
        self.self_attn = nn.MultiheadAttention(width, n_heads, dropout=dropout, batch_first=True)
        self.cross_query_norm = nn.RMSNorm(width)
        self.cross_memory_norm = nn.RMSNorm(width)
        self.cross_attn = nn.MultiheadAttention(width, n_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, latent: Tensor, memory: Tensor) -> Tensor:
        normalized = self.self_norm(latent)
        update, _ = self.self_attn(normalized, normalized, normalized, need_weights=False)
        latent = latent + self.dropout(update)
        update, _ = self.cross_attn(
            self.cross_query_norm(latent),
            self.cross_memory_norm(memory),
            self.cross_memory_norm(memory),
            need_weights=False,
        )
        latent = latent + self.dropout(update)
        return latent + self.dropout(self.ffn(self.ffn_norm(latent)))


class ContextResampler(nn.Module):
    """Compress third+wrist final vision tokens into a fixed expert-width memory."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if n_queries < 1 or n_layers < 1 or n_heads < 1:
            raise ValueError("Context queries, layers, and heads must all be >= 1.")
        if expert_width % n_heads:
            raise ValueError(
                f"context expert_width={expert_width} must be divisible by heads={n_heads}."
            )
        self.source_proj = nn.Linear(source_dim, expert_width)
        self.view_embedding = nn.Parameter(torch.zeros(2, expert_width))
        self.queries = nn.Parameter(torch.zeros(1, n_queries, expert_width))
        self.layers = nn.ModuleList(
            [ContextResamplerLayer(expert_width, n_heads, dropout) for _ in range(n_layers)]
        )
        self.final_norm = nn.RMSNorm(expert_width)
        nn.init.trunc_normal_(self.view_embedding, std=0.02)
        nn.init.trunc_normal_(self.queries, std=0.02)

    def forward(self, third_features: Tensor, wrist_features: Tensor) -> Tensor:
        dtype = self.source_proj.weight.dtype
        third = self.source_proj(third_features.to(dtype)) + self.view_embedding[0].to(dtype)
        wrist = self.source_proj(wrist_features.to(dtype)) + self.view_embedding[1].to(dtype)
        memory = torch.cat([third, wrist], dim=1)
        latent = self.queries.expand(memory.shape[0], -1, -1).to(memory.dtype)
        for layer in self.layers:
            latent = layer(latent, memory)
        return self.final_norm(latent)


class VSAFlowExpert(nn.Module):
    """PI05-warm-started action stream with optional A/B/C context reads."""
    ...
    def __init__(
        ...
        *,
        ...
        self.context_gates = nn.Parameter(torch.zeros(len(self.gemma_expert.model.layers)))
    ...
    @staticmethod
    def _project_qkv(layer: nn.Module, hidden: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        shape = (*hidden.shape[:-1], -1, layer.self_attn.head_dim)
        q = layer.self_attn.q_proj(hidden).view(shape).transpose(1, 2)
        k = layer.self_attn.k_proj(hidden).view(shape).transpose(1, 2)
    ...
    def _expert_layer_with_context(
        ...
        skill_scale: float,
        image_context: Tensor | None,
        goal_context: Tensor | None,
        image_scale: float,
        goal_scale: float,
    ) -> Tensor:
        layer = self.gemma_expert.model.layers[layer_idx]
        normalized, residual_gate = layernorm_forward(
            layer.input_layernorm, hidden, expert_cond
        )
        if skill_broadcast is not None:
            normalized = add_broadcast_condition(normalized, skill_broadcast * float(skill_scale))
        raw_q, self_k, self_v = self._project_qkv(layer, normalized)

        dummy = torch.zeros(
            raw_q.shape[0], raw_q.shape[2], raw_q.shape[-1],
            device=raw_q.device, dtype=raw_q.dtype,
        )
        cos, sin = self.gemma_expert.model.rotary_emb(dummy, position_ids)
        self_q, self_k = modeling_gemma.apply_rotary_pos_emb(
            raw_q, self_k, cos, sin, unsqueeze_dim=1
        )
        self_out, _ = modeling_gemma.eager_attention_forward(
            layer.self_attn,
            self_q,
            self_k,
            self_v,
        ...
        action_q = raw_q[:, :, -self.chunk_size :]
        context_out = None
        for context, scale in (
            (image_context, float(image_scale)),
            (goal_context, float(goal_scale)),
        ):
            ...
            update = update * scale
            context_out = update if context_out is None else context_out + update

        if context_out is not None:
            context_scale = torch.tanh(self.context_gates[layer_idx]).to(self_out.dtype)
            action_out = self_out[:, -self.chunk_size :] + context_scale * context_out
            if hidden.shape[1] == self.chunk_size:
                self_out = action_out
            else:
                self_out = torch.cat(
                    [self_out[:, : -self.chunk_size], action_out], dim=1
                )

        self_out = self_out.reshape(
            self_out.shape[0], self_out.shape[1], raw_q.shape[1] * layer.self_attn.head_dim
        )
        if self_out.dtype != layer.self_attn.o_proj.weight.dtype:
            self_out = self_out.to(layer.self_attn.o_proj.weight.dtype)
        after_attention = _gated_residual(
            hidden, layer.self_attn.o_proj(self_out), residual_gate
        )
        mlp_input, mlp_gate = layernorm_forward(
            layer.post_attention_layernorm, after_attention, expert_cond
        )
        if mlp_input.dtype != layer.mlp.up_proj.weight.dtype:
            mlp_input = mlp_input.to(layer.mlp.up_proj.weight.dtype)
        return _gated_residual(after_attention, layer.mlp(mlp_input), mlp_gate)
    ...
    def velocity(
        ...
        *,
        ...
        expert_cond = self._expert_cond(time, state, z_norm)
        skill_broadcast = self._skill_broadcast(z_norm)
        hidden = action
        for layer_idx in range(len(self.gemma_expert.model.layers)):
            if self.training and self._gradient_checkpointing:
                def layer_forward(value: Tensor, index: int = layer_idx) -> Tensor:
                    return self._expert_layer_with_context(
                        index,
                        value,
                        attn,
                        positions,
                ...
                hidden = torch.utils.checkpoint.checkpoint(
                    layer_forward, hidden, use_reentrant=False, preserve_rng_state=False
                )
            else:
                hidden = self._expert_layer_with_context(
                    layer_idx,
                    hidden,
                    attn,
                    positions,
        ...
        hidden, _ = layernorm_forward(self.gemma_expert.model.norm, hidden, expert_cond)
...
class FSQQueryTerminator(nn.Module):
    ...
    def image_features(
        self, third: Tensor | None, wrist: Tensor | None
    ) -> tuple[Tensor, Tensor]:
        return self._image_features(third), self._image_features(wrist)

    def _prepare_image_tokens(
        self,
        third: Tensor | None,
        wrist: Tensor | None,
        features: tuple[Tensor, Tensor] | None = None,
    ) -> Tensor:
        # Exactly Stage-1 cond's visual contract: shared backbone, shared projection, concatenation.
        tokens = self.image_features(third, wrist) if features is None else features
        return torch.cat(
    ...
    def forward(
        ...
        image_features: tuple[Tensor, Tensor] | None = None,
    ) -> tuple[Tensor, Tensor]:
        image_tokens = self._prepare_image_tokens(third, wrist, image_features)
...
class SplineFSQAEConfig:
    ...
    context_queries: int = 16
    context_layers: int = 4
    context_heads: int = 8
    context_dropout: float = 0.0
    a_skill_scale: float = 1.0
    b_skill_scale: float = 0.5
    c_skill_scale: float = 0.5
    b_image_scale: float = 1.0
    c_image_scale: float = 0.5
    c_goal_scale: float = 1.0
    ...
    wrong_goal_relative_margin: float = 0.05
...
class SplineFSQAE(nn.Module):
    ...
    def __init__(self, cfg: SplineFSQAEConfig):
        ...
        if cfg.wrong_goal_relative_margin < 0.0:
            raise ValueError("wrong_goal_relative_margin must be >=0.")
        ...
        context_kwargs = {
            "source_dim": self.terminator.visual_dim,
            "expert_width": self.action_expert.width,
            "n_queries": cfg.context_queries,
            "n_layers": cfg.context_layers,
            "n_heads": cfg.context_heads,
            "dropout": cfg.context_dropout,
        }
        self.image_context = ContextResampler(**context_kwargs)
        self.goal_context = ContextResampler(**context_kwargs)
    ...
    def forward(
        ...
        *,
        ...
        third: Tensor | None,
        wrist: Tensor | None,
        goal_third: Tensor | None,
        goal_wrist: Tensor | None,
        ...
        if noise is None:
            noise = self.action_expert.sample_noise(tuple(actions.shape), actions.device)
        if time is None:
            time = self.action_expert.sample_time(actions.shape[0], actions.device)

        current_features = self.terminator.image_features(third, wrist)
        goal_features = self.terminator.image_features(goal_third, goal_wrist)
        image_context = self.image_context(*current_features)
        goal_context_per_skill = self.goal_context(*goal_features)
        goal_context = goal_context_per_skill.repeat_interleave(samples_per_skill, dim=0)
        detached_skill = z_sample.detach()

        residual_a = self.action_expert.flow_residual(
            expert_state,
            z_sample,
            actions,
            noise=noise,
            time=time,
            skill_scale=self.cfg.a_skill_scale,
        )
        residual_b = self.action_expert.flow_residual(
            expert_state,
            detached_skill,
            actions,
            noise=noise,
            time=time,
            skill_scale=self.cfg.b_skill_scale,
            image_context=image_context,
            image_scale=self.cfg.b_image_scale,
        )
        residual_c = self.action_expert.flow_residual(
            expert_state,
            detached_skill,
            actions,
            noise=noise,
            time=time,
            skill_scale=self.cfg.c_skill_scale,
            image_context=image_context,
            goal_context=goal_context,
            image_scale=self.cfg.c_image_scale,
            goal_scale=self.cfg.c_goal_scale,
        )

        output = {
            "z_q": z_q,
            "indices": indices,
            "flow_residual": residual_a,
            "flow_residual_a": residual_a,
            "flow_residual_b": residual_b,
            "flow_residual_c": residual_c,
        }
        if self.cfg.wrong_goal_enabled and goal_context_per_skill.shape[0] > 1:
            wrong_goal = goal_context_per_skill.roll(shifts=1, dims=0)
            wrong_goal = wrong_goal.repeat_interleave(samples_per_skill, dim=0)
            output["flow_residual_wrong_goal"] = self.action_expert.flow_residual(
                expert_state,
                detached_skill,
                actions,
                noise=noise,
        ...
        output["term_logits"] = term_logits
        return output
...
class FSQTrajectoryDataset(Dataset):
    ...
    def _sample_images(
        ...
        """Decode M current frames plus the skill endpoint in one request per camera.
        ...
        current_indices = [start + int(timestep) for timestep in sample.tolist()]
        indices = current_indices + [start + self.lengths[index] - 1]
        ...
        third = decode("observation.images.image")
        wrist = decode("observation.images.wrist_image")
        return third[:-1], wrist[:-1], third[-1], wrist[-1]
    ...
    def __getitem__(self, index: int) -> dict[str, Tensor]:
        ...
        third, wrist, goal_third, goal_wrist = self._sample_images(index, sample)
        item = {
...
def fsq_vsa_loss(
    ...
    per_sample = {
        regime: output[f"flow_residual_{regime.lower()}"][..., : cfg.action_dim]
        .square()
        .mean(dim=(1, 2))
        for regime in ("A", "B", "C")
    }
    plain = {
        regime: _per_trajectory_mean(value, bsize, m)
        for regime, value in per_sample.items()
    }
    sample_weight = torch.ones_like(per_sample["A"])
    if cfg.weighted_loss:
        ...
        progress = batch["progress"].reshape(-1).to(per_sample["A"])
        sample_weight = 1.0 + (cfg.weighted_loss_end_weight - 1.0) * progress

    def a_weighted_mean(value: Tensor) -> Tensor:
        numerator = (value * sample_weight).view(bsize, m).sum(dim=1)
        denominator = sample_weight.view(bsize, m).sum(dim=1).clamp_min(1e-8)
        return (numerator / denominator).mean()

    # Progress weighting belongs only to A: it is the sole regime whose action
    # loss reaches the FSQ encoder/codebook. Context correction objectives stay
    # position-neutral so their relative strength is stable across the skill.
    objective = {
        "A": a_weighted_mean(per_sample["A"]),
        "B": plain["B"],
        "C": plain["C"],
    }
    direct_weights = torch.tensor(
        [cfg.regime_a_weight, cfg.regime_b_weight, cfg.regime_c_weight],
        device=objective["A"].device,
        dtype=objective["A"].dtype,
    )
    direct = sum(
        direct_weights[i] * objective[regime]
        for i, regime in enumerate(("A", "B", "C"))
    ) / direct_weights.sum().clamp_min(1e-8)

    retain = 1.0 - float(cfg.ranking_relative_margin)
    rank_ab = _per_trajectory_mean(
        F.relu(per_sample["B"] - retain * per_sample["A"].detach()), bsize, m
    )
    ...
    if "flow_residual_wrong_goal" in output:
        wrong_per_sample = output["flow_residual_wrong_goal"][..., : cfg.action_dim].square().mean(dim=(1, 2))
        wrong_plain = _per_trajectory_mean(wrong_per_sample, bsize, m)
        wrong_goal = _per_trajectory_mean(
            F.relu(
                (1.0 + float(cfg.wrong_goal_relative_margin)) * per_sample["C"]
                - wrong_per_sample
            ),
            bsize,
            m,
    ...
    metrics = {
        ...
        "action_B": plain["B"].detach(),
        "action_C": plain["C"].detach(),
        "action_wrong_goal": wrong_plain.detach(),
        "objective_A": objective["A"].detach(),
        "objective_B": objective["B"].detach(),
        "objective_C": objective["C"].detach(),
        "ranking_AB": rank_ab.detach(),
        "ranking_BC": rank_bc.detach(),
        "wrong_goal": wrong_goal.detach(),
    ...
    *,
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {
                "params": list(model.image_context.parameters()) + list(model.goal_context.parameters()),
                "lr": cfg.context_lr,
                "name": "context",
            },
    ...
    def step(batch: dict[str, Tensor | None], training: bool, batch_index: int):
        ...
        goal_third = moved["goal_third"]
        goal_wrist = moved["goal_wrist"]
        noise = time = None
        if not training:
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        log = {
            ...
            **{f"val/end_{k}": v for k, v in val_end_avg.items()},
            ...
            "lr/context": optimizer.param_groups[2]["lr"],
            "lr/terminator": optimizer.param_groups[3]["lr"],
        }

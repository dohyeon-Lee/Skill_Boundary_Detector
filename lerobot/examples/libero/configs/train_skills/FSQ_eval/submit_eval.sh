#!/usr/bin/env bash
# Submit original-FSQ broadcast-scale closed-loop LIBERO evaluation.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC_DIR="${SCRIPT_DIR}/src"
CONFIG_PATH="${FSQ_EVAL_CONFIG:-${SCRIPT_DIR}/fsq_eval_config.yaml}"
...
FSQ_EVAL_ENV_SNAPSHOT="${SCRIPT_DIR}/logs/fsq_eval_env_$(date +%Y%m%d_%H%M%S)_$$.sh"
"${BOOTSTRAP_PYTHON}" "${SRC_DIR}/fsq_eval_config.py" \
  --config "${CONFIG_PATH}" --shell > "${FSQ_EVAL_ENV_SNAPSHOT}"
source "${FSQ_EVAL_ENV_SNAPSHOT}"
export FSQ_EVAL_ENV_SNAPSHOT

# Keep a source-only FSQ-local reset map. SkillVLA parquet is never consumed.
...
# Encode each unique (checkpoint, skillset) once. Scale panels share this cache.
...
echo "Submit FSQ broadcast-scale eval"
...
if [ -n "${SLURM_JOB_ID:-}" ]; then
  ...
  while IFS= read -r KEY; do
    ...
    PANEL_KEY="${KEY}" TASK_TAG="" TASKS_TOTAL="${TASKS_TOTAL}" \
      FSQ_EVAL_DIR="${SCRIPT_DIR}" FSQ_EVAL_CONFIG="${CONFIG_PATH}" \
  ...
  FSQ_EVAL_DIR="${SCRIPT_DIR}" FSQ_EVAL_CONFIG="${CONFIG_PATH}" \
    ...
    sbatch --job-name="FSQeval" --array="${ARRAY_SPEC}" \

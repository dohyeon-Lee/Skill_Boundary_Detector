#!/usr/bin/env python3
"""Closed-loop LIBERO eval for one original-FSQ broadcast-scale panel."""
...
from fsq_policy import (  # noqa: E402
    FSQExpertPolicy,
    FSQTerminator,
    OracleSkillExpertPolicy,
...
def eval_main(cfg: EvalPipelineConfig) -> None:
    ...
    if not spec_raw:
        raise ValueError("PANEL_SPEC is required for FSQ eval.")
    spec = json.loads(spec_raw)
    device = get_safe_torch_device(cfg.policy.device, log=True)
    set_seed(cfg.seed)

    policy = FSQExpertPolicy(
        spec["fsq_path"],
        device,
        broadcast_scale=float(spec["broadcast_scale"]),
        n_action_steps=int(cfg.policy.n_action_steps),
        dino_model_path=spec["dino_model_path"],
    )
    terminator = FSQTerminator(policy)
    ...
    logging.info("Making episode-exact environment for panel %s", spec["label"])
    envs = make_env(
        cfg.env,
    ...
    if project:
        try:
            ...
            wandb.init(
                ...
                name=cfg.job_name,
                config={
                    "fsq_path": spec["fsq_path"],
                    "broadcast_scale": spec["broadcast_scale"],
                },
            )
            payload = {
                f"overall/{key}": value
                for key, value in info["overall"].items()

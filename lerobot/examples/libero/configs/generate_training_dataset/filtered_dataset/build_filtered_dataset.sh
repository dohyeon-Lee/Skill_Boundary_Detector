...
for pair in ${FILTERED_SUITES}; do
  ...
  python "${SCRIPT_DIR}/remap_gripper_zero_close.py" --dataset-dir "${final}"

  echo "== ${name}: ⑤ quantile stats =="
  if [ -n "${RECOMPUTE_STATS:-}" ]; then
    python "${SCRIPT_DIR}/ensure_quantile_stats.py" --dataset "${name}" --overwrite
  else
    python "${SCRIPT_DIR}/ensure_quantile_stats.py" --dataset "${name}"
  fi

...
def task_subset_ids(cfg: dict[str, Any], set_name: str) -> list[int]:
    """Return source task ids for a selectively downloaded set, if configured."""
    raw = cfg.get("download_task_ids_by_set") or {}
    if not isinstance(raw, dict):
        raise TypeError("download_task_ids_by_set must be a mapping of set name to task ids.")
    values = raw.get(set_name)
    if values is None:
        return []
    if isinstance(values, str):
        values = [part.strip() for part in values.split(",") if part.strip()]
    if not isinstance(values, (list, tuple)):
        raise TypeError(f"download_task_ids_by_set[{set_name!r}] must be a list of integers.")
    task_ids = [int(value) for value in values]
    if not task_ids or any(value < 0 for value in task_ids):
        raise ValueError(f"download_task_ids_by_set[{set_name!r}] must contain non-negative ids.")
    if len(task_ids) != len(set(task_ids)):
        raise ValueError(f"download_task_ids_by_set[{set_name!r}] contains duplicate ids.")
    return task_ids
...
def build_settings(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # name=id,id,... pairs. Missing names are downloaded in full.
        "langgap_task_subsets": " ".join(
            f"{name}={','.join(str(task_id) for task_id in task_subset_ids(cfg, name))}"
            for name in sets
            if task_subset_ids(cfg, name)
        ),

...
# hf가 다운로드 중이면 .incomplete/.lock 을 남긴다. 다만 이 파일이 없는 것은
# "지금 전송 중이 아님"만 의미하며, 새 subset의 remote listing/검증 중일 수도 있다.
# 최종 완료 판정은 Python wrapper가 subset별 .download_complete.json으로 담당한다.
_has_pending() {
...
# watch_dir 총 바이트가 늘면 진행 중. 완료는 오직 wrapper exit 0으로 판정한다.
# pending이 없어도 자식이 LISTING_GRACE 이상 종료하지 않으면 listing/검증 stall로 재시작.
run_with_watchdog() {
  ...
  while :; do
    ...
    local last_size; last_size=$(du -sb "${watch_dir}" 2>/dev/null | cut -f1)
    last_size=${last_size:-0}
    local last_progress=${SECONDS} started=0 outcome=""
    _has_pending "${watch_dir}" && started=1
    while :; do
      ...
      if [ "${size}" -gt "${last_size}" ]; then
        ...
        # pending이 없어도 완료라고 단정하지 않는다. 새 subset을 listing 중이거나
        # wrapper가 subset별 manifest를 검증/작성 중일 수 있다. 진짜 완료는 자식 exit 0.
        local idle=$((SECONDS - last_progress))
        if [ "${idle}" -ge "${listing_grace}" ]; then
          echo "[watchdog] STALL: pending 파일 없음 + ${idle}s 무증가인데 자식이 종료하지 않음 → 그룹 ${pgid} 종료 후 재시작"
          kill -TERM -- "-${pgid}" 2>/dev/null || true
          sleep 5
          kill -KILL -- "-${pgid}" 2>/dev/null || true
          wait "${pid}" 2>/dev/null || true

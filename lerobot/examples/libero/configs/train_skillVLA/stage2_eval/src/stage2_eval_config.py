...
def _stage2_modes(raw: str) -> tuple[str, ...]:
    """Parse Stage-2 action-route panels; default includes the raw frozen-FSQ baseline."""
    aliases = {
        "a": "a", "full": "a", "connected": "a",
        "b": "b", "severed": "b", "drop_vlm": "b",
        "vsa": "vsa", "pure_vsa": "vsa", "stage1_vsa": "vsa",
        "fsq": "fsq", "expert": "fsq", "frozen_fsq": "fsq", "frozen_expert": "fsq",
    }
    items = [s.strip().lower() for s in str(raw or "a,b,vsa,fsq").split(",") if s.strip()]
    modes = tuple(aliases.get(s, "") for s in items)
    if not modes or any(not s for s in modes) or len(set(modes)) != len(modes):
        raise ValueError(
            "stage2 models[].modes must be a comma list without duplicates from a,b,vsa,fsq "
            "(aliases: full,severed,pure_vsa,frozen_fsq), got " + repr(raw)
        )
    return modes


def _expand_stage2_panels(models: list[dict]) -> list[dict]:
    """Expand a Stage-2 checkpoint into the requested action-route comparison panels."""
    panels: list[dict] = []
    labels = {"a": "A", "b": "B", "vsa": "Pure-VSA", "fsq": "Frozen-FSQ"}
    routes = {
        "a": (False, False),   # VLM connected; vlm_lora + cond_lora active
        "b": (True, True),     # VLM severed; cond_lora remains active
        "vsa": (True, False),  # VLM severed; Stage-2 adapters disabled
        "fsq": (False, False), # raw action_expert.* from FSQ.pt; no Stage-1/2 action modules
    }
    for model in models:
        for mode in _stage2_modes(model.get("modes", "")):
            panel = dict(model)
...
def build_settings(cfg: dict) -> dict:
    ...
    skill_source_raw = get_value(cfg, "skill_source", None)
    skill_source = (
        str(skill_source_raw).strip().lower()
        if skill_source_raw not in (None, "")
        else ("gt" if as_bool(get_value(cfg, "use_gt_skill", False)) else "pred")
    )
    if skill_source not in {"gt", "pred"}:
        raise ValueError(f"skill_source must be gt|pred, got {skill_source!r}.")
    use_gt_skill = skill_source == "gt"

    # New format: every model owns checkpoint/advance_mode and expands to A/B/Pure-VSA. The old top-level
    ...
    legacy_default_modes = (
        "a,b,vsa,fsq" if legacy_eval_dropout is None
        else ("a,b,vsa" if as_bool(legacy_eval_dropout) else "a")
    )
    ...
    if not use_gt_skill and any("fsq" in _stage2_modes(e["modes"]) for e in entries):
        raise ValueError(
            "Frozen-FSQ has no skill predictor, so models[].modes containing fsq requires skill_source=gt. "
            "For predicted-skill eval use modes: a,b,vsa."
        )
    ...
    resolved = _expand_stage2_panels(resolved_models)
    ...
    if include_stage1:
        ...
        group_sizes = [len(_stage2_modes(r.get("modes", ""))) for r in resolved_models]
        ...
        for group in group_sizes:
            ...
            lbl = f"{r0['label'].rsplit(' [', 1)[0]} [Stage1]"
            while lbl in new_l:
                lbl += "x"
            entry = dict(r0)
    ...
    settings: dict = {
        ...
        "models_per_row": int(get_value(cfg, "models_per_row", 4) or 0),

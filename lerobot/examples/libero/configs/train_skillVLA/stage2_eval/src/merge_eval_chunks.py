#!/usr/bin/env python3
"""Merge task-split eval chunks (submit_eval.sh eval_num_gpus>1) into ONE summary + index.html.

Each chunk job writes eval_info_t{a}-{b}.json into the SHARED out dir; this script (run at the END of
every chunk job — idempotent, last finisher wins with the complete set) merges them into:
  * eval_info.json   — combined per_task list + recomputed overall pc_success
  * index.html       — regenerated over ALL merged tasks (same generator the single-job path uses)
Earlier finishers produce partial merges that later finishers overwrite (tmp+mv atomic). Safe to
re-run any time; no-op if no chunk files exist.
"""
...
def main() -> None:
    ...
    ap.add_argument("--target_task", default="libero_90", help="suite name for the task-description lookup")
    ap.add_argument("--job_name", default="", help="index.html header + merged-wandb run name")
    ...
    if not chunks:
        print("merge: no eval_info_t*.json chunks found — nothing to do")
        return

    # Merge per_task across chunks (dedupe on (task_group, task_id); newer file wins).
    per_task: dict[tuple, dict] = {}
    for cj in chunks:  # sorted → later (higher-tag) files override on collision; collisions shouldn't happen
        try:
    ...
    # Regenerate index.html over the merged set (same generator as the single-job path).
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parents[6] / "src"))   # …/lerobot/src (repo root = parents[6])
        from lerobot.scripts.lerobot_skillvla_eval import _libero_task_descriptions, _write_index_html  # noqa: PLC0415
        desc = _libero_task_descriptions(args.target_task)
        _write_index_html(output_dir=out, per_task_infos=tasks, task_descriptions=desc, job_name=args.job_name)
        print("merge: index.html regenerated over all merged tasks")
    except Exception as exc:  # noqa: BLE001 — the index is cosmetic; never fail the job over it
        print(f"merge: index.html regeneration skipped: {exc}")

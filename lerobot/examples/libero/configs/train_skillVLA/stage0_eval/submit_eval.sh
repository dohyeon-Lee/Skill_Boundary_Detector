...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # stage0_eval
SRC_DIR="${SCRIPT_DIR}/src"
CONFIG_PATH="${STAGE0_EVAL_CONFIG:-${SCRIPT_DIR}/stage0_eval_config.yaml}"
...
STAGE0_EVAL_ENV_SNAPSHOT="${SCRIPT_DIR}/logs/stage0_eval_env_$(date +%Y%m%d_%H%M%S)_$$.sh"
"${BOOTSTRAP_PYTHON}" "${SRC_DIR}/stage0_eval_config.py" --config "${CONFIG_PATH}" --shell > "${STAGE0_EVAL_ENV_SNAPSHOT}"
source "${STAGE0_EVAL_ENV_SNAPSHOT}"
export STAGE0_EVAL_ENV_SNAPSHOT

if [ ! -d "${POLICY_PATH}" ]; then
  echo "PT checkpoint not found: ${POLICY_PATH}" >&2
  echo "Train it first: configs/train_skillVLA/stage0/submit_train.sh" >&2
  exit 1
fi
if [ ! -f "${BASE_FSQ}" ]; then
...
if [ -n "${SLURM_JOB_ID:-}" ]; then
  ...
  STAGE0_EVAL_DIR="${SCRIPT_DIR}" STAGE0_EVAL_CONFIG="${CONFIG_PATH}" \
  ...
  STAGE0_EVAL_DIR="${SCRIPT_DIR}" STAGE0_EVAL_CONFIG="${CONFIG_PATH}" \
    ...
    sbatch --job-name="S0eval" --array="${ARRAY_SPEC}" \

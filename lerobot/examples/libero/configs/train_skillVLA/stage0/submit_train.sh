...
CONFIG_PATH="${STAGE0_TRAIN_CONFIG:-${SCRIPT_DIR}/stage0_train_config.yaml}"
...
STAGE0_ENV_SNAPSHOT="${SCRIPT_DIR}/logs/stage0_env_$(date +%Y%m%d_%H%M%S)_$$.sh"
"${BOOTSTRAP_PYTHON}" "${SRC_DIR}/stage0_train_config.py" \
  --config "${CONFIG_PATH}" --shell > "${STAGE0_ENV_SNAPSHOT}"
...
if [ -n "${SLURM_JOB_ID:-}" ]; then
  ...
  STAGE0_TRAIN_CONFIG="${CONFIG_PATH}" STAGE0_ENV_SNAPSHOT="${STAGE0_ENV_SNAPSHOT}" \

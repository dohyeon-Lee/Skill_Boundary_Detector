...
# Usage: ./submit_train.sh [arch0|arch0_skill|arch0_skill_chunk|arch1|arch1_skill|arch1_skill_chunk]
...
if [ "$#" -gt 1 ]; then
  echo "Usage: $0 [arch0|arch0_skill|arch0_skill_chunk|arch1|arch1_skill|arch1_skill_chunk]" >&2
  exit 2
fi
ARCHITECTURE_OVERRIDE="${1:-${STAGE1_ARCHITECTURE_OVERRIDE:-}}"
if [ -n "${ARCHITECTURE_OVERRIDE}" ]; then
  case "${ARCHITECTURE_OVERRIDE}" in
    arch0|arch0_skill|arch0_skill_chunk|arch1|arch1_skill|arch1_skill_chunk) ;;
    *)

#!/usr/bin/env bash
# Submit the isolated skillVLA_real condition-Gemma Stage-1 training path.
#   (login) resolve cond config + check dataset → sbatch train_cond.sbatch
...
CONFIG_PATH="${STAGE1_COND_TRAIN_CONFIG:-${SCRIPT_DIR}/stage1_cond_train_config.yaml}"
...
eval "$("${BOOTSTRAP_PYTHON}" "${SRC_DIR}/stage1_cond_train_config.py" --config "${CONFIG_PATH}" --shell)"
...
echo "Submit Stage-1 condition Gemma (skill_expert)"
...
STAGE1_COND_TRAIN_CONFIG="${CONFIG_PATH}" \
  sbatch "${SBATCH_ARGS[@]}" "${SRC_DIR}/train_cond.sbatch"

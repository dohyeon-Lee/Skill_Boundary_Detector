#!/usr/bin/env python3
"""Resolve the isolated skillVLA_real condition-Gemma Stage-1 YAML."""
...
DEFAULT_CONFIG_PATH = _HERE.parent.parent / "stage1_cond_train_config.yaml"
...
def build_settings(config: dict) -> dict:
    ...
    architecture = str(
        _at(config, "architecture", "name", default="cond_gemma")
    ).strip()
    architecture_revision = str(
        _at(
            ...
            "architecture",
            "revision",
            default="skillvla_real_v1",
        )
    ).strip()
    if architecture != "cond_gemma":
        raise ValueError(
            "The isolated cond launcher requires architecture.name=cond_gemma."
        )
    if architecture_revision != "skillvla_real_v1":
        raise ValueError(
            "The isolated cond launcher requires "
            "architecture.revision=skillvla_real_v1."
        )

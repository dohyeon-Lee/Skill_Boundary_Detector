...
def build_settings(cfg: dict, dataset: str | None = None) -> dict:
    ...
    dp_outputs_root = root / str(get_value(cfg, "dp_outputs_root", "DP_outputs"))
    ...
    # ── DINO (step 2) ──
    patch_grid = int(get_value(cfg, "dino_patch_grid", 8))
    ...
    # ── FSQ (step 4) — locate model + build run tag ──
    ...
    fsq_run_name = (
        f"{fsq_train_dataset}_{fsq_tag}_pg{patch_grid}_image{image_token_dim}"
        f"{fsq_exp_suffix}"
    )
    ...
    run_tag = (
        f"FSQ{''.join(str(v) for v in fsq_levels)}_pg{patch_grid}_image{image_token_dim}"
    ...
    skillset_dir = work_dir / "skillset"
    ...
    settings: dict = {
        ...
        # skill-level DINO tokens (FSQ-independent → shared in _work); FSQ vectors (run-specific).
        "skill_tokens_path": work_dir / "skill_tokens.npz",

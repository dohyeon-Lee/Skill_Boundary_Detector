#!/usr/bin/env python3
"""Closed-loop eval for Stage0-pretrain using autoregressively predicted skill tokens.

Pick the model to evaluate by its OUTPUT FOLDER NAME + checkpoint (under
{project_root}/{outputs_root}/skillVLA_stage0_pretrain/). The trained policy's params (FSQ path,
...
def _stage0_modes(raw: str) -> tuple[str, ...]:
    """Parse connected/severed action routes; both use the model's predicted skill tokens."""
    aliases = {
        "a": "a", "full": "a", "connected": "a",
        "b": "b", "severed": "b", "drop_vlm": "b",
    }
    items = [s.strip().lower() for s in str(raw or "a,b").split(",") if s.strip()]
    modes = tuple(aliases.get(s, "") for s in items)
    if not modes or any(not s for s in modes) or len(set(modes)) != len(modes):
        raise ValueError(
            "stage0-pretrain models[].modes must be a comma list without duplicates from a,b "
            "(aliases: full,severed), got " + repr(raw)
        )
    return modes
...
def build_settings(cfg: dict) -> dict:
    ...
    vla_root = outputs_root / str(get_value(cfg, "models_root", "skillVLA_stage0_pretrain"))
    ...
    legacy_default_modes = "a,b" if legacy_eval_dropout is None else (
    ...
    for e in entries:
        if e["advance_mode"] != "terminator":
            raise ValueError(
                "Predicted-skill Stage0-pretrain eval requires advance_mode=terminator; "
                f"got {e['advance_mode']!r}."
            )
    ...
    if any(e["advance_mode"] == "gt" for e in entries):
        raise ValueError("advance_mode=gt is unavailable without an oracle GT skill sequence.")
    for e in entries:

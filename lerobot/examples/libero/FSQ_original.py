...
def fsq_entropy_terms(
    ...
    *,
    ...
    """BSQ-style entropy terms for the FSQ grid (the attribution-ablation twin
    of ``bsq_entropy_terms``).
    ...
    and vanishes at bin centers — the same locality as BSQ's σ(2τu) bits.
    Returns (sample_entropy, dataset_entropy) in nats; the joint dataset mode
    enumerates all prod(levels) codes in FSQ's index order (dim 0 fastest).
    """
    probs: list[Tensor] = []
    ...
    q = probs[0]
    for p in probs[1:]:
        # FSQ index = sum(level_d * stride_d) with dim 0 fastest, so later dims
        # become the slow axis of the flattened product distribution.
...
class LengthFreeSplineFSQEncoder(SplineFSQEncoder):
    """SplineFSQEncoder without the length token.

    Control points live on normalized time, so with no length token the
    absolute duration reaches z only through motion shape (path extent).
    ``lengths`` is still accepted everywhere for API compatibility but never
    enters the computation. Checkpoint shapes differ from the parent (smaller
    pos_embed, no enc_len_proj), so runs can never silently cross-load.
    """
...
class ActionSeqEncoder(nn.Module):
    ...
    ``fsq`` under the same names as SplineFSQEncoder so the BSQ swap and the
